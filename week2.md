...
<details>
  ...
  느낀점 : 우선 내가 한 미션은 난이도가 낮아 수행하기 어렵지는 않았지만, 이게 학습과 성장에 도움이 되는가에 대해서는 의문이 들었던 것 같다. 이게 나에게 있어서 도움이 된다는 생각이 들지 않으니깐 미션은 쉬웠지만 여러번 시도하지 않았던 부분도 있었다.  그래서 따로 AI 음악에 대한 이슈거리들을 찾아보면서 AI의 윤리적인 문제를 생각해봤던 것 같다.
SUNO AI가 음악을 생성할때, 무료데이터를 사용해서 학습한다라고 주장했는데 
이부분에 대해 투명한 공개를 하지 않아서 생긴 문제가 있었다.
그래서 이부분에 대해서 여러나라에서는 TDM이라는 정책을 도입했다라고 들었는데  확실히 저작권의 범위가 모호해지면서 이를 어떻게 해결할 수 있을까에 대한 부분을 면밀하게 고민해야할것같다고 느꼈다.
